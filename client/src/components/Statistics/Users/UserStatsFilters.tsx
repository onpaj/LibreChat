...
const UserStatsFilters: React.FC<UserStatsFiltersProps> = ({ params, onFilterChange }) => {
  ...
  const hasActiveFilters = params.dateFrom || params.dateTo || params.minUsage || params.maxUsage || params.includeInactive;
        ...
        <div className="flex items-center space-x-4">
          ...
          >
            ...
            {hasActiveFilters && (
              <span className="bg-blue-100 text-blue-800 text-xs font-medium px-2 py-0.5 rounded-full">
                {[params.dateFrom, params.dateTo, params.minUsage, params.maxUsage, params.includeInactive].filter(Boolean).length}
              </span>
            )}
          </Button>
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => handleQuickDateRange('90d')}
            >
              90 days
            </Button>
          </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <select
              ...
              onChange={(e) => handleSortChange(e.target.value as UserLeaderboardParams['sortBy'], params.sortOrder || 'desc')}
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => handleSortChange(params.sortBy || 'totalTokens', params.sortOrder === 'desc' ? 'asc' : 'desc')}
            >
          ...
          <div className="mt-4 flex items-center space-x-4">
            <label className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => setLocalFilters(prev => ({ ...prev, includeInactive: e.target.checked }))}
                className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
              <span className="text-sm text-gray-700">Include inactive users (no activity in 30 days)</span>
            </label>
          </div>
